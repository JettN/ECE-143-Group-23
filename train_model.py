"""
DeBERTa-v3 on Kaggle LLM Preference Task

- Uses the entire preprocessed training set (dp.df)
- Uses microsoft/deberta-v3-base as a 3-class classifier:
    0: tie
    1: model_a
    2: model_b
- Runs for 1 epoch (you can increase if you want)
- Generates submission_full.csv
- ALSO saves the trained model + tokenizer to ./saved_deberta_model

All preprocessing (JSON parsing, winner labels, expansion, etc.)
is handled in data_preprocessing.py and data_preprocssing_funcs.py.
"""
...
def main():
    """
    Main entry point: train model, evaluate, save model, and create submission.
    """
    ...
    MAX_LENGTH = 384        # sequence length; adjust if needed
    BATCH_SIZE = 6          # if you hit OOM, reduce to 4
    NUM_EPOCHS = 1          # increase to 2+ if you want later
    LR = 2e-5
    SEED = 42
    ...
    training_args = TrainingArguments(
        output_dir="./deberta_output_full",
        learning_rate=LR,
    ...
    # --------------------------------------------------------
    save_dir = "./saved_deberta_model"
    os.makedirs(save_dir, exist_ok=True)

    # Either of these works; using trainer.save_model uses the wrapped model
    trainer.save_model(save_dir)          # saves model + config
    tokenizer.save_pretrained(save_dir)   # saves tokenizer files

    print(f"\nSaved trained model + tokenizer to {save_dir}\n")
